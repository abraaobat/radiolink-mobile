...
A conventional radio still needs an interface for receive audio, transmit audio, PTT, safe signal conditioning, optional CAT/control and transport to the host. The project therefore distinguishes the **physical radio interface problem** from the **TNC/modem placement decision**.

## Decision

RadioLink adopts a **host-first TNC strategy**.

For conventional-radio development, the first reference path uses the smartphone/computer — initially macOS/Linux with Direwolf or equivalent — as the software TNC/modem.
...
The RadioLink Bridge is a dual-mode accessory.

### Mode A — host-TNC interface mode

The Bridge provides the radio edge: audio RX/TX, PTT, signal conditioning, host transport and optional CAT/control. The host performs modem/TNC processing.

### Mode B — embedded-TNC mode

After Mode A is validated, the same hardware may add AFSK modulation/demodulation, AX.25 frame handling where required, KISS and BLE KISS transport. This creates a Mobilinkd-like cable-free operating mode for Android/iOS and other hosts.

Both modes must present compatible semantics through the shared TNC abstraction so APRS, Packet and future Winlink modules do not care where the modem resides.

## Raw Bluetooth audio

Raw Bluetooth audio is not selected as the primary AFSK transport because codec compression, resampling, AGC/noise suppression, buffering, latency variation and platform-specific behavior may reduce modem reliability.

BLE KISS/data is preferred for cable-free Packet/APRS operation. USB audio/PTT is preferred as the deterministic reference path for software-TNC bring-up. Wi-Fi/raw-audio transport may be explored later but is outside the initial MVP.

## Consequences

### Positive

- uses processing power already available on phones/computers;
...
- the first conventional-radio PoC uses a USB cable between interface and desktop host;
- mobile host software-TNC support still requires platform-specific audio/PTT research;
- Bridge firmware may eventually support two operational modes.
